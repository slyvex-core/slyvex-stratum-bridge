...
This is a lightweight daemon that allows mining to a local (or remote) slyvex node using stratum-base miners.

This daemon is confirmed working with the miners below in both slyvex-only and dual-mining modes (for those that support it) on Windows/MacOs/Linux/HiveOs.
* bzminer
* lolminer
* srbminer
* teamreadminer
...
Tips appreciated: 
- [@onemorebsmith](https://github.com/onemorebsmith): `slyvex:qp9v6090sr8jjlkq7r3f4h9un5rtfhu3raknfg3cca9eapzee57jzew0kxwlp`
- [@rdugan](https://github.com/rdugan): `slyvex:qrkhyhej7h0gmmvsuf8mmufget4n4xnlwx5j360sz70q7xvu0hlaxfmt9p8j8`
...
ks_valid_share_counter{ip="192.168.0.17",miner="SRBMiner-MULTI/1.0.8",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="002"} 276
ks_valid_share_counter{ip="192.168.0.24",miner="BzMiner-v11.1.0",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="003"} 43
ks_valid_share_counter{ip="192.168.0.65",miner="BzMiner-v11.1.0",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="001"} 307
# HELP ks_worker_job_counter Number of jobs sent to the miner by worker over time
# TYPE ks_worker_job_counter counter
ks_worker_job_counter{ip="192.168.0.17",miner="SRBMiner-MULTI/1.0.8",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="002"} 3471
ks_worker_job_counter{ip="192.168.0.24",miner="BzMiner-v11.1.0",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="003"} 3399
ks_worker_job_counter{ip="192.168.0.65",miner="BzMiner-v11.1.0",wallet="slyvex:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="001"} 3425
...
* run `cd cmd/slyvexbridge;go build .`

* Modify the config file in ./cmd/bridge/config.yaml with your setup, the file comments explain the various flags

* run `./slyvexbridge` in the `cmd/slyvexbridge` directory

All-in-one (build + run) `cd cmd/slyvexbridge/;go build .;./slyvexbridge`
...
* Clone this repository using git (`git clone https://github.com/rdugan/slyvex-stratum-bridge.git`) or download and unpack the [zip file](https://github.com/rdugan/slyvex-stratum-bridge/archive/refs/heads/main.zip)

* Enter the 'slyvex-stratum-bridge' directory and type the command `docker compose -f docker-compose-all-src.yml up -d --build` [^1].  This will run the bridge assuming a local slyvexd node with default port settings, and listen on port 5555 for incoming stratum connections.  These settings can be updated in the [config.yaml](cmd/slyvexbridge/config.yaml) file, or overridden by modifying/adding/deleting the parameters in the 'command' section of the [docker-compose-all-src.yml](docker-compose-all-src.yml) file.  Additionally, Prometheus (the stats database) and Grafana (the dashboard) will be started and accessible on ports 9090 and 3000 respectively.  Once all services are running, the dashboard should be reachable at <http://127.0.0.1:3000/d/x7cE7G74k1/ksb-monitoring> with default user/pass: admin/admin
...
* Enter the 'slyvex-stratum-bridge' directory and type the command `docker compose -f docker-compose-bridge-src.yml up -d --build` [^2]. This will run the bridge assuming a local slyvexd node with default port settings, and listen on port 5555 for incoming stratum connections.  These settings can be updated in the [config.yaml](cmd/slyvexbridge/config.yaml) file, or overridden by modifying/adding/deleting the parameters in the 'command' section of the [docker-compose-bridge-src.yml](docker-compose-bridge-src.yml) file.  No further services will be enabled.

[^2]: This command builds the bridge component from source, rather than the previous behavior of pulling down a pre-built image.  You may still use the pre-built image by issuing the command `docker run -p 5555:5555 onemorebsmith/slyvex_bridge:latest`, but it is not guaranteed to be up to date, so compiling from source is the better alternative.


# Configuration

Configuration for the bridge is done via the [config.yaml](cmd/slyvexbridge/config.yaml) file in the same directory as the executable, or `./cmd/slyvexbridge` from the project root if building from source / using docker.  Available parameters are as follows:
...
# slyvexd_address: address/port of the rpc server for slyvexd, typically 28110
# For a list of public nodes, run `nslookup mainnet-dnsseed.daglabs-dev.com` 
# uncomment for to use a public node
# slyvexd_address: 46.17.104.200:28110
slyvexd_address: localhost:28110
...
# block_wait_time: time to wait since last new block message from slyvexd before
...
Config parameters can also be specificied by command line flags, which have slightly different names (these would be added in the 'command' subsection of the 'svx_bridge' section of the appropriate 'docker-compose-*.yml' file for docker installations.)  This method has precedence over the config.yaml file:
...
```
  ...
  - '-slyvex=host.docker.internal:28110' # host/port at which slyvexd node is running
